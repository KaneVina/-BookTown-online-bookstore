            ...
            <div class="p-gutter max-w-container-max mx-auto">
                <div class="flex flex-col md:flex-row md:items-end justify-between gap-gutter mb-stack-lg">
                    <div>
                        ...
                        <p class="text-body-md text-on-surface-variant mt-1">Quản lý và cập nhật tiến độ xử
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-stack-sm overflow-x-auto">
                            <a href="${pageContext.request.contextPath}/dashboard/customer-order?status=all&keyword=${keyword}"
                               class="px-4 py-2 rounded-full text-label-md whitespace-nowrap ${empty status or status == 'all' ? 'bg-primary text-white' : 'bg-background-alt hover:bg-surface-container-high transition-colors text-on-surface-variant'}">
                                Tất cả</a>
                            <a href="${pageContext.request.contextPath}/dashboard/customer-order?status=pending&keyword=${keyword}"
                               class="px-4 py-2 rounded-full text-label-md whitespace-nowrap ${status == 'pending' ? 'bg-primary text-white' : 'bg-background-alt hover:bg-surface-container-high transition-colors text-on-surface-variant'}">
                                Chờ xác nhận</a>
                            ...
                            <a href="${pageContext.request.contextPath}/dashboard/customer-order?status=completed&keyword=${keyword}"
                               class="px-4 py-2 rounded-full text-label-md whitespace-nowrap ${status == 'completed' ? 'bg-primary text-white' : 'bg-background-alt hover:bg-surface-container-high transition-colors text-on-surface-variant'}">
                                Hoàn thành</a>
                            ...
                            <a href="${pageContext.request.contextPath}/dashboard/customer-order?status=pending_refund&keyword=${keyword}"
                               class="px-4 py-2 rounded-full text-label-md whitespace-nowrap ${status == 'pending_refund' ? 'bg-primary text-white' : 'bg-background-alt hover:bg-surface-container-high transition-colors text-on-surface-variant'}">
                                Chờ hoàn tiền</a>
                            <a href="${pageContext.request.contextPath}/dashboard/customer-order?status=refunded&keyword=${keyword}"
                               class="px-4 py-2 rounded-full text-label-md whitespace-nowrap ${status == 'refunded' ? 'bg-primary text-white' : 'bg-background-alt hover:bg-surface-container-high transition-colors text-on-surface-variant'}">
                                Đã hoàn tiền</a>
                        </div>
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-surface-container-low/50 border-b border-outline-variant/10">
                                    <th class="px-gutter py-4 text-label-sm text-on-surface-variant uppercase tracking-wider font-bold">
                                        Mã đơn</th>
                                    <th class="px-gutter py-4 text-label-sm text-on-surface-variant uppercase tracking-wider font-bold">
                                        Tên khách hàng</th>
                                    <th class="px-gutter py-4 text-label-sm text-on-surface-variant uppercase tracking-wider font-bold">
                                    ...
                                    <th class="px-gutter py-4 text-label-sm text-on-surface-variant uppercase tracking-wider font-bold">
                                        Trạng thái</th>
                                    <th class="px-gutter py-4 text-label-sm text-on-surface-variant uppercase tracking-wider font-bold text-right">
                                        Thao tác</th>
                                </tr>
                            </thead>
...
<<<<<<< HEAD
                                <c:if test="${empty orderList}">
                                    <tr>
                                        <td colspan="6"
                                            class="px-gutter py-8 text-center text-on-surface-variant text-body-md">
                                    ...
                                    <tr class="hover:bg-background-alt/50 transition-colors group">
                                        ...
                                        <td class="px-gutter py-4">
                                            <div class="flex items-center gap-3">
                                                <span
                                                    class="text-body-sm font-medium">${order.customerName}</span>
                                            </div>
                                        </td>
                                                ...
                                                <c:when test="${order.status == 'cancelled'}">
                                                    <c:choose>
                                                        <c:when test="${order.paymentMethod == 'vnpay' && order.paymentStatus == 'pending_refund'}">
                                                            <span class="inline-flex items-center px-3 py-1 rounded-full bg-amber-100 text-amber-700 text-label-sm font-bold">
                                                                Chờ hoàn tiền
                                                            </span>
                                                        </c:when>
                                                        <c:when test="${order.paymentMethod == 'vnpay' && order.paymentStatus == 'refunded'}">
                                                            <span class="inline-flex items-center px-3 py-1 rounded-full bg-blue-100 text-[#134aa4] text-label-sm font-bold">
                                                                Đã hoàn tiền
                                                            </span>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <span class="inline-flex items-center px-3 py-1 rounded-full bg-error/10 text-error text-label-sm font-bold">
                                                                Hủy đơn
                                                            </span>
                                                        </c:otherwise>
                                                    </c:choose>
                                        ...
                                        <td class="px-gutter py-4 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <a href="${pageContext.request.contextPath}/dashboard/customer-order?action=detail&orderID=${order.orderID}"
                                                   class="p-2 bg-surface border border-outline-variant/30 text-primary rounded-lg hover:bg-surface-container-high transition-all inline-flex items-center active:scale-90"
                                                   title="Xem chi tiết">
                                                    <span
                    ...
                    <div
                        ...
                        <p class="text-body-sm text-on-surface-variant">Trang ${currentPage} / ${totalPages}
                        </p>
                            ...
                            <c:forEach var="i" begin="1" end="${totalPages}">
                                <c:choose>
                                    <c:when test="${i == currentPage}">
                                        <span
                                            class="w-8 h-8 flex items-center justify-center rounded-lg bg-primary text-white text-label-sm font-bold">${i}</span>
                                    </c:when>
                                    <c:otherwise>
                                        <a href="${baseUrl}&page=${i}"
                                           class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-surface-container-high text-on-surface-variant text-label-sm">${i}</a>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
